## 0.5.6
Исправлено:
 - Minimessage формат теперь поддерживается в названии и в описании предметов
 - Фикс инлайнов
 - Списки и словари теперь подставляются как присвоение в переменную (значением словаря или списка)
 - Загрузка происходит через cdn дискорда (ПОМЕНЯТЬ НА СВОЙ ВЕБХУК МОЖНО ПОМЕНЯТЬ В compilator.py, ПЕРЕМЕННАЯ webhook_url)
