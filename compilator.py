...
def minimessage_text(text1):
    """Full MiniMessage parser for Minecraft 1.20.5+ JSON Text Components."""
    
    # Заменяем <newline> на пробел, так как переносы строк в custom_name не работают
    text1 = text1.replace("<newline>", " ").replace("\n", " ")
    ...
    def new_frame(parent=None):
        # italic: False по умолчанию, чтобы имя предмета не было курсивом
        f = {
            'bold': None, 'italic': False, 'underlined': None,
            'strikethrough': None, 'obfuscated': None,
            'color': None, 'font': None, 'shadow_color': None,
        }
        if parent:
            f.update({k: v for k, v in parent.items()})
        return f

    def frame_to_props(f):
        props = {}
        if f['bold'] is not None: props['bold'] = nbtworker.Byte(1 if f['bold'] else 0)
        if f['italic'] is not None: props['italic'] = nbtworker.Byte(1 if f['italic'] else 0)
        if f['underlined'] is not None: props['underlined'] = nbtworker.Byte(1 if f['underlined'] else 0)
        if f['strikethrough'] is not None: props['strikethrough'] = nbtworker.Byte(1 if f['strikethrough'] else 0)
        if f['obfuscated'] is not None: props['obfuscated'] = nbtworker.Byte(1 if f['obfuscated'] else 0)
        if f['color'] is not None: props['color'] = nbtworker.String(f['color'])
        if f['font'] is not None: props['font'] = nbtworker.String(f['font'])
        if f.get('shadow_color') is not None:
            try:
                props['shadow_color'] = nbtworker.Int(int(f['shadow_color']))
            except: pass
        return props

    tokens = tokenize_mm(text1)
    
    stack = [new_frame()]
    nodes = []

    def make_text_node(txt, frame):
        c = nbtworker.Compound(text=nbtworker.String(txt))
        for k, v in frame_to_props(frame).items():
            c[k] = v
        return c
    ...
    for kind, value in tokens:
        ...
        if kind == 'text':
            if value:
                node = make_text_node(value, current_frame)
                nodes.append(node)
            continue
        ...
        new_f = new_frame(current_frame)
        applied = False
        special_node = None

        # --- Formatting ---
        if tag_lower == 'bold': new_f['bold'] = True; applied = True
        elif tag_lower == '/bold': new_f['bold'] = False; applied = True
        elif tag_lower in ('italic', 'i'): new_f['italic'] = True; applied = True
        elif tag_lower in ('/italic', '/i'): new_f['italic'] = False; applied = True
        elif tag_lower in ('underlined', 'underline', 'u'): new_f['underlined'] = True; applied = True
        elif tag_lower in ('/underlined', '/underline', '/u'): new_f['underlined'] = False; applied = True
        elif tag_lower in ('strikethrough', 'st'): new_f['strikethrough'] = True; applied = True
        elif tag_lower in ('/strikethrough', '/st'): new_f['strikethrough'] = False; applied = True
        elif tag_lower in ('obfuscated', 'obf'): new_f['obfuscated'] = True; applied = True
        elif tag_lower in ('/obfuscated', '/obf'): new_f['obfuscated'] = False; applied = True
        
        # --- Colors ---
        elif tag_lower in NAMED_COLORS: 
            new_f['color'] = tag_lower; applied = True
        elif tag_lower.startswith('#') and len(tag_lower) == 7: 
            new_f['color'] = tag_lower.upper(); applied = True
        elif tag_lower.startswith('color:') or tag_lower.startswith('colour:'):
            col = tag.split(':', 1)[1].strip()
            new_f['color'] = col if col.startswith('#') else col.lower()
            applied = True
            
        # --- Shadow (ARGB) ---
        elif tag_lower.startswith('shadow:'):
            val = tag.split(':', 1)[1].strip()
            parts = val.split(':')
            color_part = parts[0].strip()
            alpha_part = parts[1].strip() if len(parts) > 1 else '1.0'
            
            if color_part.lower() == 'true':
                # Дефолтная черная тень
                pass
            elif color_part.startswith('#'):
                try:
                    alpha_float = float(alpha_part)
                    # Преобразуем alpha от 0.0-1.0 в 0-255
                    alpha_int = max(0, min(255, int(alpha_float * 255)))
                    color_int = int(color_part[1:], 16)
                    # Собираем ARGB
                    argb_int = (alpha_int << 24) | (color_int & 0x00FFFFFF)
                    
                    # Конвертируем в signed int32 для NBT
                    if argb_int >= 0x80000000:
                        argb_int -= 0x100000000
                        
                    new_f['shadow_color'] = str(argb_int)
                    applied = True
                except Exception as e:
                    print(f"DEBUG MM SHADOW ERROR: {e}")

        # --- Font ---
        elif tag_lower.startswith('font:'):
            new_f['font'] = tag.split(':', 1)[1]; applied = True

        # --- Special Nodes ---
        elif tag_lower.startswith('lang:') or tag_lower.startswith('translate:'):
            key = tag.split(':', 1)[1]
            special_node = nbtworker.Compound(translate=nbtworker.String(key))
            for k, v in frame_to_props(current_frame).items(): special_node[k] = v
            
        elif tag_lower.startswith('keybind:'):
            key = tag.split(':', 1)[1]
            special_node = nbtworker.Compound(keybind=nbtworker.String(key))
            for k, v in frame_to_props(current_frame).items(): special_node[k] = v

        elif tag_lower.startswith('sprite:'):
            parts = tag.split(':', 2)
            if len(parts) == 3:
                _, atlas, path = parts
                if ':' not in atlas: atlas = 'minecraft:' + atlas
                if ':' not in path: path = 'minecraft:' + path
                special_node = nbtworker.Compound(atlas=nbtworker.String(atlas), sprite=nbtworker.String(path))
                for k, v in frame_to_props(current_frame).items(): special_node[k] = v
    ...
    # --- Assemble Result ---
    def is_empty_text(n):
        return (list(n.values.keys()) == ['text'] and n['text'].value == '')

    nodes = [n for n in nodes if not is_empty_text(n)]
    ...
    root = nodes[0]
    if len(nodes) > 1:
        root['extra'] = nbtworker.List(*nodes[1:])
            ...
            if self.object == "variable":
                if self.name == "set_value":
                    ...
                    if load_args["value"].type in {"subscript", "calling_argument", "calling_function",
                                                   "calling_object", "action", "if_else_expr"} or (
...
def _text_to_nbt(t):
    if t.value_type == Texts.MINIMESSAGE:
        return minimessage_text(t.value)
    return minecraft_text(t.value)
        ...
        if ret[1] is self:
            ...
            if self.new_args["name"] is not default_jmcc_object or self.new_args["lore"] is not default_jmcc_object or \
                ...
                if self.new_args["name"] is not default_jmcc_object:
                    self.item["components"]["minecraft:custom_name"] = _text_to_nbt(self.new_args["name"])
                if self.new_args["lore"] is not default_jmcc_object:
                    self.item["components"]["minecraft:lore"] = nbtworker.List(
                        *[_text_to_nbt(i1) for i1 in self.new_args["lore"].values])
...
def parse_from_directory(directory_path, properties=None):
    if properties is None:
        properties = {}
    global global_text
    # Делаем путь абсолютным заранее
    source = os.path.abspath(directory_path)
    if len(properties) > 0:
        Context(source).set_properties(properties)
    if (source not in Context.context) or (Context(source).compiled is False):
        # Передаем АБСОЛЮТНЫЙ путь, чтобы получить абсолютные пути к файлам
        files = get_all_jc_files(source)
                
        imports = []
        for path in files:
            # Заменяем обратные слеши на прямые и экранируем кавычки
            safe_path = path.replace("\\", "/").replace("\"", "\\\"")
            imports.append(f'import "{safe_path}"')
            
        code_text = ";".join(imports)
...
def try_upload(code):
    try:
        ...
        if isinstance(code, dict):
            raw_text = code.get("code") or code.get("text") or code.get("data")
            if not raw_text or not isinstance(raw_text, str):
                raw_text = json.dumps(code, ensure_ascii=False, separators=(',', ':'))
        else:
            raw_text = code if isinstance(code, str) else str(code)
        
        data_size = len(raw_text.encode('utf-8'))
        print(f"Размер загружаемых данных: {data_size} байт ({data_size/1024/1024:.4f} мбайт) ({len(raw_text)} символов)")
        
        max_size = 8 * 1024 * 1024
        if data_size > max_size:
            raise ValueError(f"Файл слишком большой: {data_size} байт (максимум 8 MB для Discord)")
        
        # Отправляем ТОЛЬКО файл, без параметра data
        files = {
        ...
        # Получаем прямую ссылку на файл из attachments
        if 'attachments' in result and len(result['attachments']) > 0:
            file_url = result['attachments'][0]['url']
        else:
            raise ValueError("Файл не был прикреплен к сообщению")
        
        end_time1 = time()
        print(minecraft_based_text(translate("compilator.code_uploading_time", {0: round(end_time1 - start_time1, 3)})))
        print(minecraft_based_text(translate("compilator.code_uploaded_message", {0: file_url})))
...
def get_all_jc_files(directory):
    ...
    for file1 in all_files:
        # Используем os.path.join для корректной склейки путей
        file2 = os.path.join(directory, file1)
...
DEFAULT_CONDITIONAL = {
    "action": "if_player_dummy",
    "is_inverted": False
}

def inject_conditionals(obj, default_conditional=DEFAULT_CONDITIONAL):
    """Рекурсивно добавляет conditional во все операции, где его нет."""
	
    if isinstance(obj, list):
        for item in obj:
            inject_conditionals(item, default_conditional)
    elif isinstance(obj, dict):
        # Если это операция (есть "action") и нет "conditional" — добавляем
        if "action" in obj and "conditional" not in obj:
            obj["conditional"] = dict(default_conditional)
        # Рекурсивно обходим вложенные операции и handlers
        for key in ("operations", "handlers"):
            if key in obj:
                inject_conditionals(obj[key], default_conditional)
