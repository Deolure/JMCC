    ...
    def next_token(self):
        ...
        if self.current_char is None:
            ...
            while self.current_char is not None and (self.current_char.isdigit() or 
                    (es is False and self.current_char in ("e", "E")) or 
            ...
            # Убираем trailing точку если есть (например "1." -> "1")
            if token_value.endswith("."):
                token_value = token_value[:-1]
                dot = False  # Сбрасываем флаг точки
            
            # Проверяем суффиксы типов
            if self.current_char in ("b", "B"):
                self.advance()
                return Token("Byte", int(float(token_value)))
            if self.current_char in ("s", "S"):
                self.advance()
                return Token("Short", int(float(token_value)))
            if self.current_char in ("l", "L"):
                self.advance()
                return Token("Long", int(float(token_value)))
            ...
            # Если была точка или экспонента - это дробное число (Double)
            if dot or es:
                return Token("Double", float(token_value))
            
            # Иначе целое число (Int)
...
class List:
    def __init__(self, *array):
        ...
        for i in array:
            ...
            if self.type is None:
                self.type = type(i)
            elif type(i) != self.type:  # Исправлено: сравниваем типы, а не объект с типом
                raise Exception(f"Недопустимый элмент: {i.__class__.__name__}")
    ...
    def append(self, value):
        ...
        elif type(value) != self.type:  # Исправлено
            raise Exception(f"Недопустимый элемент: {value.__class__.__name__}")
    ...
    def extend(self, values):
        ...
        for i in values.values:
            if not type(i) in (
                    Compound, Byte, Short, Int, Long, Float, Double, ByteArray, String, List, IntArray, LongArray):
                raise Exception(f"Недопустимый элемент: {i.__class__.__name__}")
            if self.type is not None and type(i) != self.type:  # Добавлена проверка
                raise Exception(f"Недопустимый элемент: {i.__class__.__name__}")
