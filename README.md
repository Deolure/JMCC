...
# JMCC
Это JMCC от донзголда (царствие ему небесное), он компилирует код в json, который используется на сервере [JustMC](justmc.ru)
Изменения указаны в [CHANGELOG.md](CHANGELOG.md).

> Оригинальный проект: [donzgold/JustMC_compilator](https://github.com/donzgold/JustMC_compilator) (скрыт или удален)
...
1. Скачать `jmcc.py` на [странице релизов GitHub](https://github.com/Deolure/JMCC/releases/)
